...
parser.add_argument('output_dir', nargs='?', default="/home/mendax/project/Datasets/space_station/test", help="Path to where the final files will be saved")
...
hdr_path = "/home/mendax/project/Datasets/space_station/background/RenderCrate-HDRI_Orbital_10_Sunset_4K.hdr"
...
for obj_idx, obj_path in enumerate(objs_path):  # 可以添加更多对象到列表中
    ...
    # 计算实际尺寸
    # 放大物体，按比例增加缩放
    scale_factor = 10  # 放大1.5倍（可以调整这个值来控制放大倍数）
    ...
    def sample_pose_func(obj: bproc.types.MeshObject):
        ...
        obj.set_rotation_euler(np.random.uniform([-np.pi/3, -np.pi/3, -np.pi/3], [np.pi/3, np.pi/3, np.pi/3]))
        # obj.set_rotation_euler([0,0,0])
        # obj.set_location(np.random.uniform([-r, -r, r], [r, r, r]))

    light_point = bproc.types.Light()
    light_point.set_location([5,5,5])
    # light_point.set_energy(3000)

    # 渲染多个相机视角
    scene_num = 10
    pose_num = 10
    for _ in range(scene_num):  # 每个对象生成10个不同的场景
        light_point.set_energy(np.random.uniform(0.8, 3.2)*1000)
        ...
        while poses < pose_num:
            location = bproc.sampler.shell(center=[0, 0, 2*r],
                                           radius_min=0.5*r,
                                           radius_max=r,
                                           elevation_min=1,
                                           elevation_max=89,
            ...
            if bproc.camera.perform_obstacle_in_view_check(cam2world_matrix, {"min": r,"max": 3*r}, bop_bvh_tree):
